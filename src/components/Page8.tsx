...
// --- Optimizations ---
// 1. Moved static destination data outside the component to create it only once.
...
const Page8 = () => {
    ...
    // 2. Memoized the dynamic title calculation to run only when the slug changes.
    const dynamicTitle = useMemo(() => {
        const currentDestination = ALL_DESTINATIONS.find(dest => dest.slug === slug);
    ...
    // 3. Memoized event handlers with useCallback for performance.
    useEffect(() => {
        const handleResize = () => {
        ...
        <div className="w-full min-h-screen bg-gray-100 font-['poppins']">
            ...
            <div className="hidden lg:block">
                <div style={{ width: "100vw", height: "100vh", overflow: "hidden", position: "relative", backgroundColor: "#F2F2F2" }}>
                    ...
                    `}</style>
                    <div style={{ position: "absolute", top: "50%", left: "50%", transform: `translate(-50%, -50%) scale(${scale * 0.95})`, width: 1512, height: 900 }}>
                        ...
                        <div style={{ position: "absolute", top: 280, left: "calc(50% - 190px/2 - 500px)", display: "flex", flexDirection: "column", alignItems: "center", zIndex: 2 }}>
                            {VISIBLE_DESTINATIONS.map((dest, index) => (
                                <div key={dest.slug} className="destination-button" onClick={() => handleDestinationClick(dest.slug)} onMouseEnter={() => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)} style={{ width: 160, padding: '10px 0', margin: '6px 0', background: "#263a48ff", boxShadow: "0px 4px 4px rgba(0,0,0,0.25)", borderRadius: 30 }}>
                                    {dest.name}
                                </div>
                            ))}
                        </div>
                        {VISIBLE_DESTINATIONS.map((dest, index) => (
                            <div key={dest.slug} className="circle-hover" onClick={() => handleDestinationClick(dest.slug)} style={{
                                position: "absolute",
                                ...
                                borderRadius: "50%", boxShadow: "0px 4px 12px rgba(0,0,0,0.3)",
                                transition: "all 0.3s ease-in-out", zIndex: 2
                            }} />
                        ))}
                    </div>
                </div>
