...
const Page5: React.FC<{ data?: any }> = ({ data }) => {
  ...
  let attractionsList;
  if (content.PlacesToExplore) {
    attractionsList = content.PlacesToExplore;
  } else {
    attractionsList = Object.keys(content.attractions).map(key => ({
  ...
  const allBottomIcons = attractionsList.slice(3);

  const currentImage = selectedItem ? selectedItem.imageUrl : content.backgroundUrl;
  const currentContent = selectedItem ? { title: selectedItem.name, description: selectedItem.description } : { title: content.mainTitle, description: content.mainDescription };
    ...
    <div className="w-full min-h-screen lg:h-screen relative overflow-x-hidden" >
      <style>{`
        ...
        @media (max-width: 1024px) {
          ...
          .page5-text-content h1 {
            font-size: 48px !important;
            line-height: 1.2 !important;
          }
          .page5-text-content p {
          ...
          .page5-right-column {
            ...
            top: 0 !important;
            /* CORRECTED: Shifted left from 50% to 40% */
            left: 40% !important;
      ...
      {/* CORRECTED: Restored original desktop container classes and styles */}
